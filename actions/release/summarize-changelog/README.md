...
- Can generate summary directly through an OpenAI-compatible API using the `openai` npm package.
...
| `llm-model`            | Optional OpenAI-compatible model used to generate summary from `llm-prompt`.       | false    | `""`                        |
| `llm-api-key`          | Optional API key for the OpenAI-compatible model provider.                         | false    | `""`                        |
| `llm-base-url`         | Optional OpenAI-compatible API base URL.                                           | false    | `https://api.openai.com/v1` |
